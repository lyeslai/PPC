...
class DeadCollector (val terminaux:List[Terminal], electionActor: ActorRef) extends Actor with Broadcast {
  ...
  val deathThreshold: Int = 10 // number of checks before a node is considered failed
  ...
  var aloneCheckingInterval: Int = 10
  var aloneCheckingCounter: Int = 25
  ...
  override def receive: Receive = {
    ...
    case Check_leader => {
      if (!musiciansAlive.contains(leader)) {
    ...
    musiciansAlive.foreach { case (musicianId, lastAlive) =>
      ...
      if (now.getTime - lastAlive.getTime > deathThreshold * checkInterval.toMillis) {
        ...
        terminaux.foreach { t =>
          ...
          val terminalLeader = terminaux.find(t => t.id == leader)
          val leaderaddr = s"akka.tcp://MozartSystem${terminalLeader.get.id}@${terminalLeader.get.ip}:${terminalLeader.get.port}/user/Musicien${terminalLeader.get.id}/ChefOrchestre"
          context.actorSelection(address) ! Musician_dead(musicianId)
          context.actorSelection(leaderaddr) ! Musician_dead(musicianId)
        }
