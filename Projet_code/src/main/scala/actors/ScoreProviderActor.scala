...
case class RequestScore(diceRoll: Int)

class ScoreProviderActor(musicianRef: ActorRef) extends Actor {
  private var sequenceCounter = 0
  
  private val part1 = Array(
  ...
  def receive: Receive = {
    case RequestScore(diceRoll) =>
      val measureTable = part1 ++ part2

      // Get measure number from table using dice roll and sequence counter
      val measureNumber = measureTable(diceRoll)(sequenceCounter) - 1
      
      // Advance sequence counter, reset after 8 measures
      sequenceCounter = (sequenceCounter + 1) % 8
      
      // Forward measure request to the musician
      musicianRef ! RequestMeasure(measureNumber)
  }
}
